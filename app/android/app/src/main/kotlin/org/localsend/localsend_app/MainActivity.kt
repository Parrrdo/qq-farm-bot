...
class MainActivity : FlutterActivity() {
    ...
    /**
     * Show a file in the file manager by opening its parent directory.
     *
     * Opens the parent directory of the given file path so the user can see the
     * file in context, rather than opening the file itself.
     */
    private fun showInFolder(filePath: String) {
        val file = File(filePath)
        if (!file.exists()) {
            openUri(context, filePath)
            return
        }

        val parentDir = file.parentFile ?: file

        // Approach 1: Use FileProvider to get a content URI for the parent directory
        try {
        ...
        // Approach 2: Open the parent directory using file:// URI
        try {
            val intent = Intent(Intent.ACTION_VIEW).apply {
            ...
            // Final fallback: use openUri which handles the file with proper MIME type
            openUri(context, filePath)
        }
    }
