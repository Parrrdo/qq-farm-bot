...
Future<void> openFolder({
  ...
  } else if (fileName != null && checkPlatform([TargetPlatform.android])) {
    // Android: use native showInFolder which properly opens the file via FileProvider
    // with correct MIME type, instead of passing a directory path to OpenFilex.open()
    final filePath = '$folderPath/$fileName'.replaceAll('//', '/');
    await android_channel.showInFolder(filePath: filePath);
  } else {
    // only open folder
